...
time_t Efi2Time(EFI_TIME *EfiBDtime){
	Efi2Tm( EfiBDtime, &BDTime);
	return mktime( &BDTime);
}

void Tm2Efi(struct tm *BT,EFI_TIME*ET){
...
EFI_TIME*Time2Efi(time_t CalTime){
	struct tm *IT;
	EFI_TIME  *ET   = NULL;
	IT = gmtime(&CalTime);
	if(IT != NULL) {
  ...
  )
{
  ...
  */
  ...
  y = EPOCH_YEAR;
  while (days < 0 || days >= (INT32) year_lengths[yleap = isleap(y)]) {
    time_t /*INTN*/  newy;
  ...
  for (tmp->tm_mon = 0; days >= (INT32) ip[tmp->tm_mon]; ++(tmp->tm_mon))
    days = days - (INT32) ip[tmp->tm_mon];
