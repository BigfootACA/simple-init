...
{
	Reinst *inst = NULL, *split = NULL, *jump = NULL;
	unsigned int i;
...
int regexp_exec(Reprog *prog, const char *sp, Resub *sub, int eflags)
{
	Resub scratch;
	int i;
...
int main(int argc, char **argv)
{
	...
	if (argc > 1) {
		p = regexp_comp(argv[1], 0, &error);
		if (!p) {
			fprintf(stderr, "regexp_comp: %s\n", error);
			return 1;
		}
