...
char *__canonicalize_dm_name(const char *prefix, const char *ptname)
{
	...
	snprintf(path, sizeof(path), "%s"_PATH_SYS_BLOCK"/%s/dm/name", prefix, ptname);
	if (!(f = fopen(path, "re")))
		return NULL;
...
char *canonicalize_path_restricted(const char *path)
{
	...
	case 0:
		...
		write(pipes[1], (char *) &len, sizeof(len));
		if (canonical)
			write(pipes[1], canonical, len);
		exit(0);
	default:
		break;
	}
	...
	/* read size or -errno */
	if (read(pipes[0], (char *) &len, sizeof(len)) != sizeof(len))
		goto done;
	if (len < 0) {
		errsv = -len;
		goto done;
	...
	/* read path */
	if (read(pipes[0], canonical, len) != len) {
		errsv = errno;
		goto done;
	}
	canonical[len] = '\0';
