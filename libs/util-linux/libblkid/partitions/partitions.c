...
int blkid_partitions_need_typeonly(blkid_probe pr)
{
	...
	return chn && chn->data && chn->binary ? 0 : 1;
}
...
{
	return partition_get_logical_type(par) == 'P' ? 1 : 0;
}
...
{
	return partition_get_logical_type(par) == 'E' ? 1 : 0;
}
...
{
	return partition_get_logical_type(par) == 'L' ? 1 : 0;
}
