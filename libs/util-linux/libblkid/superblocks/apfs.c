...
static int probe_apfs(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	if (le16toh(sb->type) != APFS_CONTAINER_SUPERBLOCK_TYPE)
		return BLKID_PROBE_NONE;

	if (le16toh(sb->subtype) != APFS_CONTAINER_SUPERBLOCK_SUBTYPE)
		return BLKID_PROBE_NONE;
	 ...
	 * is published.
	 */
	if (le32toh(sb->block_size) != APFS_STANDARD_BLOCK_SIZE)
		return BLKID_PROBE_NONE;

	if (blkid_probe_set_uuid(pr, sb->uuid) < 0)
		return BLKID_PROBE_NONE;

	blkid_probe_set_block_size(pr, le32toh(sb->block_size));
