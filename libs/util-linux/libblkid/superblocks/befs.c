...
#define FS16_TO_CPU(value, fs_is_le) (fs_is_le ? le16toh(value) \
							: be16toh(value))
#define FS32_TO_CPU(value, fs_is_le) (fs_is_le ? le32toh(value) \
							: be32toh(value))
#define FS64_TO_CPU(value, fs_is_le) (fs_is_le ? le64toh(value) \
							: be64toh(value))
...
static int probe_befs(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	if (le32toh(bs->magic1) == SUPER_BLOCK_MAGIC1
		&& le32toh(bs->magic2) == SUPER_BLOCK_MAGIC2
		&& le32toh(bs->magic3) == SUPER_BLOCK_MAGIC3
		&& le32toh(bs->fs_byte_order) == SUPER_BLOCK_FS_ENDIAN) {
		fs_le = 1;
		version = "little-endian";
	} else if (be32toh(bs->magic1) == SUPER_BLOCK_MAGIC1
		&& be32toh(bs->magic2) == SUPER_BLOCK_MAGIC2
		&& be32toh(bs->magic3) == SUPER_BLOCK_MAGIC3
		&& be32toh(bs->fs_byte_order) == SUPER_BLOCK_FS_ENDIAN) {
		fs_le = 0;
		version = "big-endian";
	} else
		return BLKID_PROBE_NONE;
