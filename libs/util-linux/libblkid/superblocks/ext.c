		...
		blkid_probe pr, uint32_t *fc, uint32_t *fi, uint32_t *frc)
{
	...
	if (!es)
		return NULL;
	if (fc)
		*fc = le32toh(es->s_feature_compat);
	if (fi)
		*fi = le32toh(es->s_feature_incompat);
	if (frc)
		*frc = le32toh(es->s_feature_ro_compat);
...
static void ext_get_info(blkid_probe pr, int ver, struct ext2_super_block *es)
{
	...
	if (le32toh(es->s_feature_compat) & EXT3_FEATURE_COMPAT_HAS_JOURNAL)
		blkid_probe_set_uuid_as(pr, es->s_journal_uuid, "EXT_JOURNAL");

	if (ver != 2 && (chn->flags & BLKID_SUBLKS_SECTYPE) &&
	    ((le32toh(es->s_feature_incompat) & EXT2_FEATURE_INCOMPAT_UNSUPPORTED) == 0))
		blkid_probe_set_value(pr, "SEC_TYPE",
				(unsigned char *) "ext2",
				sizeof("ext2"));

	blkid_probe_sprintf_version(pr, "%u.%u",
		le32toh(es->s_rev_level),
		le16toh(es->s_minor_rev_level));

	if (le32toh(es->s_log_block_size) < 32)
		blkid_probe_set_block_size(pr, 1024U << le32toh(es->s_log_block_size));
}
		...
		const struct blkid_idmag *mag __attribute__((__unused__)))
{
	...
	if (!(le32toh(es->s_flags) & EXT2_FLAGS_TEST_FILESYS))
		return 1;
