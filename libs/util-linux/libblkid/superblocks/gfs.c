...
static int probe_gfs(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	if (be32toh(sbd->sb_fs_format) == GFS_FORMAT_FS &&
	    be32toh(sbd->sb_multihost_format) == GFS_FORMAT_MULTI)
	{
		if (*sbd->sb_locktable)
...
static int probe_gfs2(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	if (gfs2_format_is_valid(be32toh(sbd->sb_fs_format)) &&
	    gfs2_multiformat_is_valid(be32toh(sbd->sb_multihost_format)))
	{
		if (*sbd->sb_locktable)
			blkid_probe_set_label(pr,
				(unsigned char *) sbd->sb_locktable,
		...
		blkid_probe_set_block_size(pr, be32toh(sbd->sb_bsize));
		return 0;
	}
	return 1;
}
