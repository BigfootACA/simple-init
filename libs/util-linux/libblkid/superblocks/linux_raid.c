...
static int probe_raid0(blkid_probe pr, uint64_t off)
{
	...
	if (le32toh(mdp0->md_magic) == MD_SB_MAGIC) {
		uuid.ints[0] = bswap_32(mdp0->set_uuid0);
		if (le32toh(mdp0->minor_version) >= 90) {
			uuid.ints[1] = bswap_32(mdp0->set_uuid1);
			uuid.ints[2] = bswap_32(mdp0->set_uuid2);
			uuid.ints[3] = bswap_32(mdp0->set_uuid3);
		}
		ma = le32toh(mdp0->major_version);
		mi = le32toh(mdp0->minor_version);
		pa = le32toh(mdp0->patch_version);
		size = le32toh(mdp0->size);

	} else if (be32toh(mdp0->md_magic) == MD_SB_MAGIC) {
		uuid.ints[0] = mdp0->set_uuid0;
		if (be32toh(mdp0->minor_version) >= 90) {
			uuid.ints[1] = mdp0->set_uuid1;
			uuid.ints[2] = mdp0->set_uuid2;
			uuid.ints[3] = mdp0->set_uuid3;
		}
		ma = be32toh(mdp0->major_version);
		mi = be32toh(mdp0->minor_version);
		pa = be32toh(mdp0->patch_version);
		size = be32toh(mdp0->size);
	} else
		return 1;
...
static int probe_raid1(blkid_probe pr, off_t off)
{
	...
	if (!mdp1)
		return errno ? -errno : 1;
	if (le32toh(mdp1->magic) != MD_SB_MAGIC)
		return 1;
	if (le32toh(mdp1->major_version) != 1U)
		return 1;
	if (le64toh(mdp1->super_offset) != (uint64_t) off >> 9)
		return 1;
