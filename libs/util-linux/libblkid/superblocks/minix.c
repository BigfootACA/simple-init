...
#define minix_bswap_16(doit, num)	((uint16_t) (doit ? bswap_16(num) : num))
#define minix_bswap_32(doit, num)	((uint32_t) (doit ? bswap_32(num) : num))
		...
		const struct blkid_idmag *mag __attribute__((__unused__)))
{
	...
	version = get_minix_version(data, &bswap_me);
	switch (version) {
	case 1:
	case 2: {
		const struct minix_super_block *sb = (const struct minix_super_block *) data;

		uint16_t state = minix_bswap_16(bswap_me, sb->s_state);
		if ((state & (MINIX_VALID_FS | MINIX_ERROR_FS)) != state)
			return 1;

		zones = version == 2 ? minix_bswap_32(bswap_me, sb->s_zones) :
				       minix_bswap_16(bswap_me, sb->s_nzones);
		ninodes = minix_bswap_16(bswap_me, sb->s_ninodes);
		imaps   = minix_bswap_16(bswap_me, sb->s_imap_blocks);
		zmaps   = minix_bswap_16(bswap_me, sb->s_zmap_blocks);
		firstz  = minix_bswap_16(bswap_me, sb->s_firstdatazone);
	...
	case 3: {
		...
		zones = minix_bswap_32(bswap_me, sb->s_zones);
		ninodes = minix_bswap_32(bswap_me, sb->s_ninodes);
		imaps   = minix_bswap_16(bswap_me, sb->s_imap_blocks);
		zmaps   = minix_bswap_16(bswap_me, sb->s_zmap_blocks);
		firstz  = minix_bswap_16(bswap_me, sb->s_firstdatazone);
		zone_size = sb->s_log_zone_size;
		block_size = minix_bswap_16(bswap_me, sb->s_blocksize);
