...
static int nilfs_valid_sb(blkid_probe pr, struct nilfs_super_block *sb, int is_bak)
{
	...
	if (!sb || le16toh(sb->s_magic) != NILFS_SB_MAGIC)
		return 0;
	...
	crc = ul_crc32(le32toh(sb->s_crc_seed), (unsigned char *)sb, sumoff);
	crc = ul_crc32(crc, sum, 4);
	crc = ul_crc32(crc, (unsigned char *)sb + crc_start, bytes - crc_start);

	return blkid_probe_verify_csum(pr, crc, le32toh(sb->s_sum));
}
		...
		const struct blkid_idmag *mag __attribute__((__unused__)))
{
	...
	swp = valid[1] && (!valid[0] ||
			   le64toh(sbp->s_last_cno) >
			   le64toh(sbb->s_last_cno));
	sb = swp ? sbb : sbp;
	...
	blkid_probe_sprintf_version(pr, "%u", le32toh(sb->s_rev_level));
	...
	if (le32toh(sb->s_log_block_size) < 32)
		blkid_probe_set_block_size(pr, 1024U << le32toh(sb->s_log_block_size));
