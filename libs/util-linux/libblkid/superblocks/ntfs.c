...
static int __probe_ntfs(blkid_probe pr, const struct blkid_idmag *mag, int save_info)
{
	 ...
	 */
	sector_size = le16toh(ns->bpb.sector_size);
	...
	if ((uint16_t) le16toh(ns->bpb.sector_size) *
			sectors_per_cluster > NTFS_MAX_CLUSTER_SIZE)
		return 1;

	/* Unused fields must be zero */
	if (le16toh(ns->bpb.reserved_sectors)
	    || le16toh(ns->bpb.root_entries)
	    || le16toh(ns->bpb.sectors)
	    || le16toh(ns->bpb.sectors_per_fat)
	    || le32toh(ns->bpb.large_sectors)
	    || ns->bpb.fats)
		return 1;
	...
	nr_clusters = le64toh(ns->number_of_sectors) / sectors_per_cluster;

	if ((le64toh(ns->mft_cluster_location) > nr_clusters) ||
	    (le64toh(ns->mft_mirror_cluster_location) > nr_clusters))
		return 1;


	off = le64toh(ns->mft_cluster_location) * sector_size *
		sectors_per_cluster;
	...
	attr_off = le16toh(mft->attrs_offset);

	while (attr_off + sizeof(struct file_attribute) <= mft_record_size &&
	       attr_off <= le32toh(mft->bytes_allocated)) {

		uint32_t attr_len;
		struct file_attribute *attr;

		attr = (struct file_attribute *) (buf_mft + attr_off);
		attr_len = le32toh(attr->len);
		if (!attr_len)
			break;

		if (le32toh(attr->type) == (uint32_t) MFT_RECORD_ATTR_END)
			break;
		if (le32toh(attr->type) == (uint32_t) MFT_RECORD_ATTR_VOLUME_NAME) {
			unsigned int val_off = le16toh(attr->value_offset);
			unsigned int val_len = le32toh(attr->value_len);
	...
	blkid_probe_sprintf_uuid(pr,
			...
			"%016" PRIX64, le64toh(ns->volume_serial));
	return 0;
}
