...
static int probe_ocfs2(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	blkid_probe_sprintf_version(pr, "%u.%u",
		le16toh(osb->s_major_rev_level),
		le16toh(osb->s_minor_rev_level));

	if (le32toh(osb->s_blocksize_bits) < 32)
		blkid_probe_set_block_size(pr, 1U << le32toh(osb->s_blocksize_bits));
