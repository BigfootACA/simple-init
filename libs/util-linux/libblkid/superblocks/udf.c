...
#define UDF_LVIDIU_OFFSET(vd) (sizeof((vd).tag) + sizeof((vd).type.logical_vol_integ) + 2 * 4 * le32toh((vd).type.logical_vol_integ.num_partitions))
#define UDF_LVIDIU_LENGTH(vd) (le32toh((vd).type.logical_vol_integ.imp_use_length))
...
anchor:
		...
		if (le32toh(vd->tag.location) == s_off / pbs[i] + 256) {
			type = le16toh(vd->tag.id);
			if (type == TAG_ID_AVDP)
				goto real_blksz;
		}
		...
		if (le32toh(vd->tag.location) == s_off / pbs[i] + 512) {
			type = le16toh(vd->tag.id);
			if (type == TAG_ID_AVDP)
				goto real_blksz;
		}
...
real_blksz:
	...
	count = le32toh(vd->type.anchor.length) / bs;
	loc = le32toh(vd->type.anchor.location);
	...
	for (b = 0; b < count; b++) {
		...
		if (type == 0)
			break;
		if (le32toh(vd->tag.location) != loc + b)
			break;
		if (type == TAG_ID_TD)
			break;
		if (type == TAG_ID_PVD) {
			...
			if (!lvid_len || !lvid_loc) {
				uint32_t num_partition_maps = le32toh(vd->type.logical.num_partition_maps);
				/* ECMA-167 3/10.6.12: If num_partition_maps is 0, then no LVID is specified */
				if (num_partition_maps) {
					lvid_len = le32toh(vd->type.logical.lvid_length);
					lvid_loc = le32toh(vd->type.logical.lvid_location);
				}
			}
			if (!udf_rev) {
				...
				if (strncmp(vd->type.logical.domain_id, "*OSTA UDF Compliant", sizeof(vd->type.logical.domain_id)) == 0)
					udf_rev = le16toh(vd->type.logical.udf_rev);
			}
	...
	if (lvid_loc && lvid_len >= sizeof(*vd)) {
		...
		type = le16toh(vd->tag.id);
		if (type == TAG_ID_LVID &&
		    le32toh(vd->tag.location) == lvid_loc &&
			...
			lvidiu_udf_rev = le16toh(lvidiu->min_udf_read_rev);
			if (lvidiu_udf_rev && udf_rev < lvidiu_udf_rev)
				udf_rev = lvidiu_udf_rev;
			lvidiu_udf_rev = le16toh(lvidiu->min_udf_write_rev);
			if (lvidiu_udf_rev && udf_rev < lvidiu_udf_rev)
				udf_rev = lvidiu_udf_rev;
		}
	}
