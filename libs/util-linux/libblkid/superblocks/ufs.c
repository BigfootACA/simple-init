		...
		const struct blkid_idmag *mag __attribute__((__unused__)))
{
	...
	for (i = 0; i < ARRLEN(offsets); i++) {
		...
		magBE = be32toh(ufs->fs_magic);
		magLE = le32toh(ufs->fs_magic);

		for (y = 0; y < ARRLEN(mags); y++) {
...
found:
	...
	if (!is_be)
		blkid_probe_set_block_size(pr, le32toh(ufs->fs_fsize));
	else
		blkid_probe_set_block_size(pr, be32toh(ufs->fs_fsize));
