			...
			uint32_t *cluster_count, uint32_t *fat_size)
{
	...
	reserved =  le16toh(ms->ms_reserved);
	sect_count = unaligned_le16(&ms->ms_sectors);

	if (sect_count == 0)
		sect_count = le32toh(ms->ms_total_sect);

	fat_length = le16toh(ms->ms_fat_length);
	if (fat_length == 0)
		fat_length = le32toh(vs->vs_fat32_length);
...
static int probe_vfat(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	if (ms->ms_fat_length) {
		...
		uint32_t entries = ((uint64_t) le32toh(vs->vs_fat32_length)
					* sector_size) / sizeof(uint32_t);
		uint32_t next = le32toh(vs->vs_root_cluster);
		...
		while (next && next < entries && --maxloop) {
			...
			/* set next cluster */
			next = le32toh(*((uint32_t *) buf)) & 0x0fffffff;
		}
		 ...
		 */
		fsinfo_sect = le16toh(vs->vs_fsinfo_sector);
		if (fsinfo_sect) {
