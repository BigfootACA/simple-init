			 ...
			 struct xfs_super_block *to)
{
	...
	to->sb_magicnum = be32toh(from->sb_magicnum);
	to->sb_blocksize = be32toh(from->sb_blocksize);
	to->sb_dblocks = be64toh(from->sb_dblocks);
	to->sb_rblocks = be64toh(from->sb_rblocks);
	to->sb_rextents = be64toh(from->sb_rextents);
	to->sb_logstart = be64toh(from->sb_logstart);
	to->sb_rootino = be64toh(from->sb_rootino);
	to->sb_rbmino = be64toh(from->sb_rbmino);
	to->sb_rsumino = be64toh(from->sb_rsumino);
	to->sb_rextsize = be32toh(from->sb_rextsize);
	to->sb_agblocks = be32toh(from->sb_agblocks);
	to->sb_agcount = be32toh(from->sb_agcount);
	to->sb_rbmblocks = be32toh(from->sb_rbmblocks);
	to->sb_logblocks = be32toh(from->sb_logblocks);
	to->sb_versionnum = be16toh(from->sb_versionnum);
	to->sb_sectsize = be16toh(from->sb_sectsize);
	to->sb_inodesize = be16toh(from->sb_inodesize);
	to->sb_inopblock = be16toh(from->sb_inopblock);
	...
	to->sb_icount = be64toh(from->sb_icount);
	to->sb_ifree = be64toh(from->sb_ifree);
	to->sb_fdblocks = be64toh(from->sb_fdblocks);
	to->sb_frextents = be64toh(from->sb_frextents);
}
...
static int xfs_verify_sb(struct xfs_super_block *ondisk)
{
	...
	/* beXXtoh(), but don't convert UUID and fsname! */
...
static int probe_xfs(blkid_probe pr, const struct blkid_idmag *mag)
{
	...
	blkid_probe_set_block_size(pr, be16toh(xs->sb_sectsize));
	return 0;
}
...
static int xlog_valid_rec_header(struct xlog_rec_header *rhead)
{
	...
	if (rhead->h_magicno != htobe32(XLOG_HEADER_MAGIC_NUM))
		return 0;

	if (!rhead->h_version ||
            (be32toh(rhead->h_version) & (~XLOG_VERSION_OKBITS)))
		return 0;

	/* LR body must have data or it wouldn't have been written */
	hlen = be32toh(rhead->h_len);
	if (hlen <= 0 || hlen > INT_MAX)
		return 0;

	if (rhead->h_fmt != htobe32(XLOG_FMT_LINUX_LE) &&
	    rhead->h_fmt != htobe32(XLOG_FMT_LINUX_BE) &&
	    rhead->h_fmt != htobe32(XLOG_FMT_IRIX_BE))
		return 0;
