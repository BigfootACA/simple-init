...
#define BSD_DKMAXTYPES	(ARRLEN(bsd_dktypenames) - 1)

static struct fdisk_parttype bsd_fstypes[] = {
        {BSD_FS_UNUSED, "unused", 0, 0, 0},
	{BSD_FS_SWAP,   "swap", 0, 0, 0},
	{BSD_FS_V6,     "Version 6", 0, 0, 0},
	{BSD_FS_V7,     "Version 7", 0, 0, 0},
	{BSD_FS_SYSV,   "System V", 0, 0, 0},
	{BSD_FS_V71K,   "4.1BSD", 0, 0, 0},
	{BSD_FS_V8,     "Eighth Edition", 0, 0, 0},
	{BSD_FS_BSDFFS, "4.2BSD", 0, 0, 0},
#ifdef __alpha__
	{BSD_FS_EXT2,   "ext2", 0, 0, 0},
#else
	{BSD_FS_MSDOS,  "MS-DOS", 0, 0, 0},
#endif
	{BSD_FS_BSDLFS, "4.4LFS", 0, 0, 0},
	{BSD_FS_OTHER,  "unknown", 0, 0, 0},
	{BSD_FS_HPFS,   "HPFS", 0, 0, 0},
	{BSD_FS_ISO9660,"ISO-9660", 0, 0, 0},
	{BSD_FS_BOOT,   "boot", 0, 0, 0},
	{BSD_FS_ADOS,   "ADOS", 0, 0, 0},
	{BSD_FS_HFS,    "HFS", 0, 0, 0},
	{BSD_FS_ADVFS,	"AdvFS", 0, 0, 0},
	{ 0, NULL, 0, 0, 0}
};
...
int fdisk_bsd_write_bootstrap(struct fdisk_context *cxt)
{
	...
	if (write(cxt->dev_fd, l->bsdbuffer, BSD_BBSIZE) != BSD_BBSIZE) {
...
static int bsd_readlabel(struct fdisk_context *cxt)
{
	...
	if (lseek(cxt->dev_fd, offset, SEEK_SET) == -1)
		return -1;
	if (read(cxt->dev_fd, l->bsdbuffer, sizeof(l->bsdbuffer)) < 0)
...
static int bsd_write_disklabel(struct fdisk_context *cxt)
{
	...
	if (write(cxt->dev_fd, l->bsdbuffer, sizeof(l->bsdbuffer))!=sizeof(l->bsdbuffer)) {
		fdisk_warn(cxt, _("cannot write %s"), cxt->dev_path);
		return -errno;
	}
	sync_disks(cxt);
...
static const struct fdisk_field bsd_fields[] =
{
	...
	{ FDISK_FIELD_TYPE,	"Type",		  8,	0 },
	{ FDISK_FIELD_FSIZE,	"Fsize",	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_BSIZE,	"Bsize",	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_CPG,	"Cpg",		  5,	FDISK_FIELDFL_NUMBER }
};
...
struct fdisk_label *fdisk_new_bsd_label(struct fdisk_context *cxt __attribute__ ((__unused__)))
{
	...
	lb->nparttypes = ARRLEN(bsd_fstypes) - 1;

	lb->fields = bsd_fields;
	lb->nfields = ARRLEN(bsd_fields);
