...
static struct fdisk_parttype dos_parttypes[] = {
	#define MBR_TYPE(_code,_name){.code=_code,.name=_name}
...
static int dos_verify_disklabel(struct fdisk_context *cxt)
{
	...
	if (!nerrors) {
		...
		fdisk_warnx(cxt, _("%d errors detected."), nerrors);
			     ...
			     size_t *partno)
{
	 ...
	 * is gap in front if the first partition or between used partitions. */
	for (i = 0; i < 4; i++) {
...
static const struct fdisk_field dos_fields[] =
{
	...
	/* expert mode */
	{ FDISK_FIELD_SADDR,	("Start-C/H/S"),   1,   FDISK_FIELDFL_NUMBER | FDISK_FIELDFL_DETAIL },
	{ FDISK_FIELD_EADDR,	("End-C/H/S"),     1,   FDISK_FIELDFL_NUMBER | FDISK_FIELDFL_DETAIL },
	{ FDISK_FIELD_ATTR,	("Attrs"),	   2,   FDISK_FIELDFL_NUMBER | FDISK_FIELDFL_DETAIL }
...
struct fdisk_label *fdisk_new_dos_label(struct fdisk_context *cxt __attribute__ ((__unused__)))
{
	...
	lb->nparttypes = ARRLEN(dos_parttypes) - 1;
	lb->parttype_cuts = dos_parttype_cuts;
	lb->nparttype_cuts = ARRLEN(dos_parttype_cuts);

	lb->fields = dos_fields;
	lb->nfields = ARRLEN(dos_fields);
