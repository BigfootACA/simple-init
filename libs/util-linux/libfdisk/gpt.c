...
/* gpt_entry macros */
#define gpt_partition_start(_e)		le64toh((_e)->lba_start)
#define gpt_partition_end(_e)		le64toh((_e)->lba_end)
...
{
	uid->time_low = bswap_32(uid->time_low);
	uid->time_mid = bswap_16(uid->time_mid);
	uid->time_hi_and_version = bswap_16(uid->time_hi_and_version);
}
...
static const char *gpt_get_header_revstr(struct gpt_header *header)
{
	...
	switch (le32toh(header->revision)) {
...
{
	return gpt->ents + le32toh(gpt->pheader->sizeof_partition_entry) * i;
}
...
static inline struct gpt_entry *gpt_zeroize_entry(struct fdisk_gpt_label *gpt, size_t i)
{
	return (struct gpt_entry *) memset(gpt_get_entry_ptr(gpt, i),
			0, le32toh(gpt->pheader->sizeof_partition_entry));
}
...
{
	return (size_t) le32toh(gpt->pheader->npartition_entries);
}
...
{
	uint32_t esz = hdr ? le32toh(hdr->sizeof_partition_entry) :
...
{
	return gpt_calculate_sizeof_entries(hdr, le32toh(hdr->npartition_entries), sz);
}
...
static int gpt_mknew_pmbr(struct fdisk_context *cxt)
{
	...
	pmbr->partition_record[0].starting_lba = htole32(1);
	pmbr->partition_record[0].size_in_lba  =
		htole32((uint32_t) MIN( cxt->total_sectors - 1ULL, 0xFFFFFFFFULL) );
...
static int gpt_fix_alternative_lba(struct fdisk_context *cxt, struct fdisk_gpt_label *gpt)
{
	...
	nents = le32toh(p->npartition_entries);

	/* reference from primary to backup */
	p->alternative_lba = htole64(cxt->total_sectors - 1ULL);
	...
	b->partition_entry_lba = htole64(x);
	...
	p->last_usable_lba  = htole64(x);
	b->last_usable_lba  = htole64(x);
...
static uint64_t gpt_calculate_minimal_size(struct fdisk_context *cxt, struct fdisk_gpt_label *gpt)
{
	...
	gpt_calculate_last_lba(hdr, le32toh(hdr->npartition_entries), &x, cxt);
...
static int gpt_possible_minimize(struct fdisk_context *cxt, struct fdisk_gpt_label *gpt)
{
	...
	return le64toh(hdr->alternative_lba) > (total - 1ULL);
}
				    ...
				    struct gpt_header *header, uint64_t lba)
{
	...
	header->my_lba = htole64(lba);

	if (lba == GPT_PRIMARY_PARTITION_TABLE_LBA) {
		/* primary */
		header->alternative_lba = htole64(cxt->total_sectors - 1ULL);
		header->partition_entry_lba = htole64(2ULL);

	} else {
		/* backup */
		uint64_t x = 0;
		gpt_calculate_alternative_entries_lba(header,
				le32toh(header->npartition_entries), &x, cxt);

		header->alternative_lba = htole64(GPT_PRIMARY_PARTITION_TABLE_LBA);
		header->partition_entry_lba = htole64(x);
	}
}
			   ...
			   struct gpt_header *src)
{
	...
	if (res->my_lba == GPT_PRIMARY_PARTITION_TABLE_LBA)
		res->partition_entry_lba = htole64(2ULL);
	else {
		uint64_t esz = (uint64_t) le32toh(src->npartition_entries) * sizeof(struct gpt_entry);
		uint64_t esects = (esz + cxt->sector_size - 1) / cxt->sector_size;

		res->partition_entry_lba = htole64(cxt->total_sectors - 1ULL - esects);
	}
			    ...
			    struct gpt_header *header, uint64_t lba)
{
	...
	header->signature = htole64(GPT_HEADER_SIGNATURE);
	header->revision  = htole32(GPT_HEADER_REVISION_V1_00);
	 ...
	 */
	header->size = htole32(sizeof(struct gpt_header)
	...
	header->npartition_entries     = htole32(nents);
	header->sizeof_partition_entry = htole32(sizeof(struct gpt_entry));

	header->first_usable_lba = htole64(first);
	header->last_usable_lba  = htole64(last);
...
static int valid_pmbr(struct fdisk_context *cxt)
{
	...
	if (le16toh(pmbr->signature) != MSDOS_MBR_SIGNATURE)
		goto done;
	...
	if (pmbr->partition_record[part].starting_lba !=
	    htole32(GPT_PRIMARY_PARTITION_TABLE_LBA))
		goto done;
	 ...
	 */
	if (ret == GPT_MBR_PROTECTIVE) {
		uint64_t sz_lba = (uint64_t) le32toh(pmbr->partition_record[part].size_in_lba);
					 ...
					 struct gpt_header *header)
{
	...
	offset = (off_t) le64toh(header->partition_entry_lba) *
		       cxt->sector_size;
...
static inline uint32_t gpt_header_count_crc32(struct gpt_header *header)
{
        return count_crc32((unsigned char *) header,		/* buffer */
			le32toh(header->size),		/* size of buffer */
...
static void gpt_recompute_crc(struct gpt_header *header, unsigned char *ents)
{
	...
	header->partition_entry_array_crc32 =
			htole32( gpt_entryarr_count_crc32(header, ents) );

	header->crc32 = htole32( gpt_header_count_crc32(header) );
}
...
{
	uint32_t orgcrc = le32toh(header->crc32),
...
static int gpt_check_entryarr_crc(struct gpt_header *header, unsigned char *ents)
{
	...
	return gpt_entryarr_count_crc32(header, ents) ==
			le32toh(header->partition_entry_array_crc32);
}
...
static int gpt_check_lba_sanity(struct fdisk_context *cxt, struct gpt_header *header)
{
	...
	fu = le64toh(header->first_usable_lba);
	lu = le64toh(header->last_usable_lba);
...
{
	return header->signature == htole64(GPT_HEADER_SIGNATURE);
}
		...
		const char **name, uint64_t *offset, size_t *size)
{
	...
	*size = 0;
	...
	case 2:
		*name = _("GPT Entries");
		gpt = self_label(cxt);
		*offset = (uint64_t) le64toh(gpt->pheader->partition_entry_lba) *
				     cxt->sector_size;
		return gpt_sizeof_entries(gpt->pheader, size);
	case 3:
		*name = _("GPT Backup Entries");
		gpt = self_label(cxt);
		*offset = (uint64_t) le64toh(gpt->bheader->partition_entry_lba) *
				     cxt->sector_size;
		return gpt_sizeof_entries(gpt->bheader, size);
	case 4:
		*name = _("GPT Backup Header");
		gpt = self_label(cxt);
		*offset = (uint64_t) le64toh(gpt->pheader->alternative_lba) * cxt->sector_size;
		*size = sizeof(struct gpt_header);
		break;
	default:
...
static int gpt_get_disklabel_item(struct fdisk_context *cxt, struct fdisk_labelitem *item)
{
	...
	case GPT_LABELITEM_ENTRIESLASTLBA:
		...
		gpt_calculate_sectorsof_entries(h,
				le32toh(h->npartition_entries), &x, cxt);
		item->data.num64 = le64toh(h->partition_entry_lba) + x - 1;
		break;
	case GPT_LABELITEM_ENTRIESALLOC:
		item->name = _("Allocated partition entries");
		item->type = 'j';
		item->data.num64 = le32toh(h->npartition_entries);
		break;
	default:
...
static uint64_t find_first_available(struct fdisk_gpt_label *gpt, uint64_t start)
{
	...
	fu = le64toh(gpt->pheader->first_usable_lba);
	lu = le64toh(gpt->pheader->last_usable_lba);
...
static uint64_t find_last_free(struct fdisk_gpt_label *gpt, uint64_t start)
{
	...
	nearest_start = le64toh(gpt->pheader->last_usable_lba);
...
static uint64_t find_last_free_sector(struct fdisk_gpt_label *gpt)
{
	...
	last = le64toh(gpt->pheader->last_usable_lba);
	do {
		size_t i;
...
static int gpt_probe_label(struct fdisk_context *cxt)
{
	...
	if (gpt->pheader)
		...
		gpt->bheader = gpt_read_header(cxt,
					le64toh(gpt->pheader->alternative_lba),
					NULL);
	else
	...
	if (gpt->minimize == 0 &&
	    (le64toh(gpt->pheader->alternative_lba) > cxt->total_sectors ||
	     le64toh(gpt->pheader->alternative_lba) < cxt->total_sectors - 1ULL)) {
...
static int gpt_write(struct fdisk_context *cxt, off_t offset, void *buf, size_t count)
{
	...
	if (write(cxt->dev_fd, buf, count) != (ssize_t)count)
		return -errno;
				...
				struct gpt_header *header, unsigned char *ents)
{
	...
	return gpt_write(cxt,
			(off_t) le64toh(header->partition_entry_lba) * cxt->sector_size,
			ents, esz);
}
...
static int gpt_write_pmbr(struct fdisk_context *cxt)
{
	...
	if (cxt->total_sectors - 1ULL > 0xFFFFFFFFULL)
		pmbr->partition_record[0].size_in_lba = htole32(0xFFFFFFFF);
	else
		pmbr->partition_record[0].size_in_lba =
			htole32((uint32_t) (cxt->total_sectors - 1ULL));
...
static int gpt_write_disklabel(struct fdisk_context *cxt)
{
	...
	if (le64toh(gpt->pheader->alternative_lba) > cxt->total_sectors)
		goto err0;

	/* check that the backup header is properly placed */
	if (le64toh(gpt->pheader->alternative_lba) < cxt->total_sectors - 1ULL)
		goto err0;
	...
	if (gpt_write_header(cxt, gpt->bheader,
			     le64toh(gpt->pheader->alternative_lba)) != 0)
		goto err1;
...
static int gpt_verify_disklabel(struct fdisk_context *cxt)
{
	...
	if (le64toh(gpt->pheader->my_lba) != GPT_PRIMARY_PARTITION_TABLE_LBA) {
		nerror++;
		fdisk_warnx(cxt, _("MyLBA mismatch with real position at primary header."));
	}
	if (gpt->bheader && le64toh(gpt->bheader->my_lba) != last_lba(cxt)) {
		nerror++;
	 ...
	 */
	if (gpt->bheader && (le64toh(gpt->pheader->my_lba) !=
			     le64toh(gpt->bheader->alternative_lba))) {
		nerror++;
	...
	if (!nerror) { /* yay :-) */
		...
		fdisk_warnx(cxt, _("%d errors detected."), nerror);
		...
		size_t *partno)
{
	...
	disk_f = find_first_available(gpt, le64toh(pheader->first_usable_lba));
	...
	if (max_l == le64toh(gpt->pheader->last_usable_lba))
	...
	if (user_l > le64toh(pheader->last_usable_lba)) {
		fdisk_warnx(cxt, _("The last usable GPT sector is %ju, but %ju is requested."),
				le64toh(pheader->last_usable_lba), user_l);
		rc = -EINVAL;
		goto done;
	}

	if (user_f < le64toh(pheader->first_usable_lba)) {
		fdisk_warnx(cxt, _("The first usable GPT sector is %ju, but %ju is requested."),
				le64toh(pheader->first_usable_lba), user_f);
		rc = -EINVAL;
		goto done;
	}
	...
	e->lba_end = htole64(user_l);
	e->lba_start = htole64(user_f);
...
int fdisk_gpt_set_npartitions(struct fdisk_context *cxt, uint32_t nents)
{
	...
	old_nents = le32toh(gpt->pheader->npartition_entries);
	if (old_nents == nents)
	...
	if (rc) {
		uint32_t entry_size = le32toh(gpt->pheader->sizeof_partition_entry);
	...
	gpt->pheader->npartition_entries = htole32(nents);
	gpt->bheader->npartition_entries = htole32(nents);

	/* usable LBA addresses will have changed */
	fdisk_set_first_lba(cxt, first_usable);
	fdisk_set_last_lba(cxt, last_usable);
	gpt->pheader->first_usable_lba = htole64(first_usable);
	gpt->bheader->first_usable_lba = htole64(first_usable);
	gpt->pheader->last_usable_lba = htole64(last_usable);
	gpt->bheader->last_usable_lba = htole64(last_usable);

	/* The backup header must be recalculated */
	gpt_mknew_header_common(cxt, gpt->bheader, le64toh(gpt->pheader->alternative_lba));
		...
		uint64_t *attrs)
{
	...
	*attrs = le64toh(gpt_get_entry(gpt, partnum)->attrs);
	return 0;
}
		...
		uint64_t attrs)
{
	...
	gpt_get_entry(gpt, partnum)->attrs = htole64(attrs);
...
static int gpt_reset_alignment(struct fdisk_context *cxt)
{
	...
	if (h) {
		...
		cxt->first_lba = le64toh(h->first_usable_lba);
		cxt->last_lba  = le64toh(h->last_usable_lba);
	} else {
...
{
	/* basic */
	{ FDISK_FIELD_DEVICE,	("Device"),	 10,	0 },
	{ FDISK_FIELD_START,	("Start"),	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_END,	("End"),	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_SECTORS,	("Sectors"),	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_SIZE,	("Size"),	  5,	FDISK_FIELDFL_NUMBER | FDISK_FIELDFL_EYECANDY },
	{ FDISK_FIELD_TYPE,	("Type"),	0.1,	FDISK_FIELDFL_EYECANDY },
	/* expert */
	{ FDISK_FIELD_TYPEID,	("Type-UUID"), 36,	FDISK_FIELDFL_DETAIL },
	{ FDISK_FIELD_UUID,	("UUID"),	 36,	FDISK_FIELDFL_DETAIL },
	{ FDISK_FIELD_NAME,	("Name"),	0.2,	FDISK_FIELDFL_DETAIL },
	{ FDISK_FIELD_ATTR,	("Attrs"),	  0,	FDISK_FIELDFL_DETAIL }
};
...
struct fdisk_label *fdisk_new_gpt_label(struct fdisk_context *cxt __attribute__ ((__unused__)))
{
	...
	lb->nparttypes = ARRLEN(gpt_parttypes);
	lb->parttype_cuts = gpt_parttype_cuts;
	lb->nparttype_cuts = ARRLEN(gpt_parttype_cuts);

	lb->fields = gpt_fields;
	lb->nfields = ARRLEN(gpt_fields);
