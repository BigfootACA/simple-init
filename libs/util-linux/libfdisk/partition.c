	...
	while (fdisk_table_next_partition(tb, &itr, &pa) == 0) {
		...
		if (!last) {
			if (start >= pa->start &&  start < pa->start + pa->size) {
				if (fdisk_partition_is_freespace(pa) || pa == cur) last = pa;
				else break;


				*maxsz = pa->size - (start - pa->start);
			}
		} else if (!fdisk_partition_is_freespace(pa) && pa != cur) break;
		 else {
			last = pa;
			*maxsz = pa->size - (start - pa->start);
		}
			...
			struct fdisk_partition *res, struct fdisk_partition *tpl)
{
	...
	if (!FDISK_IS_UNDEF(start)) {
		...
		int inv = 1;
		if (area != cur) inv = 0;
		else if (area && fdisk_partition_is_freespace(area)) inv = 0;
		else if (!area && start >= cxt->first_lba && start < cxt->first_lba + (cxt->grain / cxt->sector_size)) inv = 0;
		if (inv) goto erange;
	} else {
