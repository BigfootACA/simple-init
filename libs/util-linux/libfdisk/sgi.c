...
{
	{SGI_TYPE_VOLHDR,	("SGI volhdr"),0,0,0},
	{SGI_TYPE_TRKREPL,	("SGI trkrepl"),0,0,0},
	{SGI_TYPE_SECREPL,	("SGI secrepl"),0,0,0},
	{SGI_TYPE_SWAP,		("SGI raw"),0,0,0},
	{SGI_TYPE_BSD,		("SGI bsd"),0,0,0},
	{SGI_TYPE_SYSV,		("SGI sysv"),0,0,0},
	{SGI_TYPE_ENTIRE_DISK,	("SGI volume"),0,0,0},
	{SGI_TYPE_EFS,		("SGI efs"),0,0,0},
	{SGI_TYPE_LVOL,		("SGI lvol"),0,0,0},
	{SGI_TYPE_RLVOL,	("SGI rlvol"),0,0,0},
	{SGI_TYPE_XFS,		("SGI xfs"),0,0,0},
	{SGI_TYPE_XFSLOG,	("SGI xfslog"),0,0,0},
	{SGI_TYPE_XLV,		("SGI xlv"),0,0,0},
	{SGI_TYPE_XVM,		("SGI xvm"),0,0,0},
	{MBR_LINUX_SWAP_PARTITION, ("Linux swap"),0,0,0},
	{MBR_LINUX_DATA_PARTITION, ("Linux native"),0,0,0},
	{MBR_LINUX_LVM_PARTITION, ("Linux LVM"),0,0,0},
	{MBR_LINUX_RAID_PARTITION, ("Linux RAID"),0,0,0},
	{0, NULL,0,0,0}
};
...
static struct sgi_info *sgi_new_info(void)
{
	...
	info->magic = htobe32(SGI_INFO_MAGIC);
	info->b1 = htobe32(-1);
	info->b2 = htobe16(-1);
	info->b3 = htobe16(1);
	...
	info->check1816 = htobe16(18 * 256 + 16);
...
int fdisk_sgi_create_info(struct fdisk_context *cxt)
{
	...
	sgilabel->volume[0].block_num = htobe32(2);
	sgilabel->volume[0].num_bytes = htobe32(sizeof(struct sgi_info));
		...
		unsigned int f, unsigned int l)
{
	...
	for (i = 0; i < ARRLEN(sgi->freelist); i++) {
...
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be16toh(sgilabel->devparam.nsect);
}

static int sgi_get_ntrks(struct fdisk_context *cxt)
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be16toh(sgilabel->devparam.ntrks);
}
...
static int sgi_probe_label(struct fdisk_context *cxt)
{
	...
	if (be32toh(sgilabel->magic) != SGI_LABEL_MAGIC) {
		sgi->header = NULL;
		return 0;
	}
...
static int sgi_get_disklabel_item(struct fdisk_context *cxt, struct fdisk_labelitem *item)
{
	...
	case SGI_LABELITEM_PCYLCOUNT:
		...
		item->type = 'j';
		item->data.num64 = (uint64_t) be16toh(sgiparam->pcylcount);
		break;
	...
	case SGI_LABELITEM_ILFACT:
		...
		item->type = 'j';
		item->data.num64 = (uint64_t) be16toh(sgiparam->ilfact);
		break;
...
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be32toh(sgilabel->partitions[i].first_block);
}

static unsigned int sgi_get_num_sectors(struct fdisk_context *cxt, int i)
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be32toh(sgilabel->partitions[i].num_blocks);
}

static int sgi_get_sysid(struct fdisk_context *cxt, int i)
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be32toh(sgilabel->partitions[i].type);
}

static int sgi_get_bootpartition(struct fdisk_context *cxt)
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be16toh(sgilabel->root_part_num);
}

static int sgi_get_swappartition(struct fdisk_context *cxt)
{
	struct sgi_disklabel *sgilabel = self_disklabel(cxt);
	return be16toh(sgilabel->swap_part_num);
}
...
static int sgi_check_bootfile(struct fdisk_context *cxt, const char *name)
{
	...
	if (sz > sizeof(sgilabel->boot_file)) {
		fdisk_warnx(cxt, _("Name of bootfile is too long: %zu bytes maximum."),
...
static int sgi_write_disklabel(struct fdisk_context *cxt)
{
	...
	sgilabel = self_disklabel(cxt);
	sgilabel->csum = 0;
	sgilabel->csum = htobe32(sgi_pt_checksum(sgilabel));


	if (lseek(cxt->dev_fd, 0, SEEK_SET) < 0)
		goto err;
	if (write(cxt->dev_fd, sgilabel, DEFAULT_SECTOR_SIZE) != DEFAULT_SECTOR_SIZE)
		goto err;
	if (!strncmp((char *) sgilabel->volume[0].name, "sgilabel", 8)) {
		 ...
		 */
		int infostartblock
			= be32toh(sgilabel->volume[0].block_num);
		...
		if (!info)
			goto err;
		if (write(cxt->dev_fd, info, sizeof(*info)) != sizeof(*info))
			goto err;
	}
...
static int verify_disklabel(struct fdisk_context *cxt, int verbose)
{
	...
	for (i=1, start=0; i<sortcount; i++) {
		...
		if (start > sgi_get_start_sector(cxt, Index[i])) {
			if (verbose)
				fdisk_info(cxt,
					   _("Partitions %d and %d overlap by %d sectors."),
			     ...
			     unsigned int start, unsigned int length, int sys)
{
	...
	sgilabel = self_disklabel(cxt);
	sgilabel->partitions[i].type = htobe32(sys);
	sgilabel->partitions[i].num_blocks = htobe32(length);
	sgilabel->partitions[i].first_block = htobe32(start);
...
static int sgi_create_disklabel(struct fdisk_context *cxt)
{
	...
	sgilabel->magic = htobe32(SGI_LABEL_MAGIC);
	sgilabel->root_part_num = htobe16(0);
	sgilabel->swap_part_num = htobe16(1);
	...
	sgilabel->devparam.pcylcount	= htobe16(cxt->geom.cylinders);
	sgilabel->devparam.head_vol0	= htobe16(0);
	sgilabel->devparam.ntrks	= htobe16(cxt->geom.heads);
	/* tracks/cylinder (heads) */
	sgilabel->devparam.cmd_tag_queue_depth	= (0);
	sgilabel->devparam.unused0			= (0);
	sgilabel->devparam.unused1	= htobe16(0);
	sgilabel->devparam.nsect	= htobe16(cxt->geom.sectors);
	/* sectors/track */
	sgilabel->devparam.bytes	= htobe16(cxt->sector_size);
	sgilabel->devparam.ilfact	= htobe16(1);
	sgilabel->devparam.flags	= htobe32(
			SGI_DEVPARAM_TRACK_FWD
			| SGI_DEVPARAM_IGNORE_ERRORS
			| SGI_DEVPARAM_RESEEK);
	sgilabel->devparam.datarate	= htobe32(0);
	sgilabel->devparam.retries_on_error	= htobe32(1);
	sgilabel->devparam.ms_per_word		= htobe32(0);
	sgilabel->devparam.xylogics_gap1	= htobe16(0);
	sgilabel->devparam.xylogics_syncdelay	= htobe16(0);
	sgilabel->devparam.xylogics_readdelay	= htobe16(0);
	sgilabel->devparam.xylogics_gap2	= htobe16(0);
	sgilabel->devparam.xylogics_readgate	= htobe16(0);
	sgilabel->devparam.xylogics_writecont	= htobe16(0);
		...
		struct fdisk_partition *pa)
{
	...
	if (pa->type) {
		...
		sgilabel->partitions[i].type = htobe32(t->code);
	}

	if (fdisk_partition_has_start(pa))
		sgilabel->partitions[i].first_block = htobe32(pa->start);
	if (fdisk_partition_has_size(pa))
		sgilabel->partitions[i].num_blocks = htobe32(pa->size);
...
static int sgi_toggle_partition_flag(struct fdisk_context *cxt, size_t i, unsigned long flag)
{
	...
	case SGI_FLAG_BOOT:
		sgilabel->root_part_num =
			be16toh(sgilabel->root_part_num) == i ?
			0 : htobe16(i);
		fdisk_label_set_changed(cxt->label, 1);
		break;
	case SGI_FLAG_SWAP:
		sgilabel->swap_part_num =
			be16toh(sgilabel->swap_part_num) == i ?
			0 : htobe16(i);
		fdisk_label_set_changed(cxt->label, 1);
		break;
	default:
...
static const struct fdisk_field sgi_fields[] =
{
	...
	{ FDISK_FIELD_ATTR,	("Attrs"),	  0,	FDISK_FIELDFL_NUMBER }
};
...
struct fdisk_label *fdisk_new_sgi_label(struct fdisk_context *cxt __attribute__ ((__unused__)))
{
	...
	lb->nparttypes = ARRLEN(sgi_parttypes) - 1;
	lb->fields = sgi_fields;
	lb->nfields = ARRLEN(sgi_fields);
