...
static struct fdisk_parttype sun_parttypes[] = {
	{SUN_TAG_UNASSIGNED, ("Unassigned"),0,0,0},
	{SUN_TAG_BOOT, ("Boot"),0,0,0},
	{SUN_TAG_ROOT, ("SunOS root"),0,0,0},
	{SUN_TAG_SWAP, ("SunOS swap"),0,0,0},
	{SUN_TAG_USR, ("SunOS usr"),0,0,0},
	{SUN_TAG_WHOLEDISK, ("Whole disk"),0,0,0},
	{SUN_TAG_STAND, ("SunOS stand"),0,0,0},
	{SUN_TAG_VAR, ("SunOS var"),0,0,0},
	{SUN_TAG_HOME, ("SunOS home"),0,0,0},
	{SUN_TAG_ALTSCTR, ("SunOS alt sectors"),0,0,0},
	{SUN_TAG_CACHE, ("SunOS cachefs"),0,0,0},
	{SUN_TAG_RESERVED, ("SunOS reserved"),0,0,0},
	{SUN_TAG_LINUX_SWAP, ("Linux swap"),0,0,0},
	{SUN_TAG_LINUX_NATIVE, ("Linux native"),0,0,0},
	{SUN_TAG_LINUX_LVM, ("Linux LVM"),0,0,0},
	{SUN_TAG_LINUX_RAID, ("Linux raid autodetect"),0,0,0},
	{ 0, NULL,0,0,0 }
};
		...
		uint64_t start, uint64_t stop, uint16_t sysid)
{
	...
	sunlabel->vtoc.infos[i].id = htobe16(sysid);
	sunlabel->vtoc.infos[i].flags = htobe16(0);
	sunlabel->partitions[i].start_cylinder =
		htobe32(start / (cxt->geom.heads * cxt->geom.sectors));
	sunlabel->partitions[i].num_sectors = htobe32(stop - start);
...
static int sun_probe_label(struct fdisk_context *cxt)
{
	...
	if (be16toh(sunlabel->magic) != SUN_LABEL_MAGIC) {
		sun->header = NULL;
		return 0;		/* failed */
	}
	...
	cxt->geom.heads = be16toh(sunlabel->nhead);
	cxt->geom.cylinders = be16toh(sunlabel->ncyl);
	cxt->geom.sectors = be16toh(sunlabel->nsect);

	/* we have on label geom, but user has to win */
	if (fdisk_has_user_device_geometry(cxt))
		fdisk_apply_user_device_properties(cxt);

	if (be32toh(sunlabel->vtoc.version) != SUN_VTOC_VERSION) {
		fdisk_warnx(cxt, _("Detected sun disklabel with wrong version [%d]."),
			be32toh(sunlabel->vtoc.version));
		need_fixing = 1;
	}
	if (be32toh(sunlabel->vtoc.sanity) != SUN_VTOC_SANITY) {
		fdisk_warnx(cxt, _("Detected sun disklabel with wrong vtoc.sanity [0x%08x]."),
			be32toh(sunlabel->vtoc.sanity));
		need_fixing = 1;
	}
	if (be16toh(sunlabel->vtoc.nparts) != SUN_MAXPARTITIONS) {
		fdisk_warnx(cxt, _("Detected sun disklabel with wrong vtoc.nparts [%u]."),
			be16toh(sunlabel->vtoc.nparts));
		need_fixing = 1;
	}
	if (need_fixing) {
		...
		sunlabel->vtoc.version = htobe32(SUN_VTOC_VERSION);
		sunlabel->vtoc.sanity = htobe32(SUN_VTOC_SANITY);
		sunlabel->vtoc.nparts = htobe16(SUN_MAXPARTITIONS);
		sunlabel->csum = 0;
...
static int sun_create_disklabel(struct fdisk_context *cxt)
{
	...
	sunlabel->magic = htobe16(SUN_LABEL_MAGIC);
	sunlabel->vtoc.version = htobe32(SUN_VTOC_VERSION);
	sunlabel->vtoc.sanity = htobe32(SUN_VTOC_SANITY);
	sunlabel->vtoc.nparts = htobe16(SUN_MAXPARTITIONS);
	...
	sunlabel->acyl   = htobe16(0);
	sunlabel->pcyl   = htobe16(cxt->geom.cylinders);
	sunlabel->rpm    = htobe16(5400);
	sunlabel->intrlv = htobe16(1);
	sunlabel->apc    = htobe16(0);

	sunlabel->nhead  = htobe16(cxt->geom.heads);
	sunlabel->nsect  = htobe16(cxt->geom.sectors);
	sunlabel->ncyl   = htobe16(cxt->geom.cylinders);

	snprintf((char *) sunlabel->label_id, sizeof(sunlabel->label_id),
		 "Linux cyl %ju alt %u hd %u sec %ju",
		 (uintmax_t) cxt->geom.cylinders,
		 be16toh(sunlabel->acyl),
		 cxt->geom.heads,
	...
	if (!cxt->script) {
		...
		set_partition(cxt, 1,
			  ...
			  SUN_TAG_LINUX_SWAP);
		sunlabel->vtoc.infos[1].flags |= htobe16(SUN_FLAG_UNMNT);
...
static int sun_toggle_partition_flag(struct fdisk_context *cxt, size_t i, unsigned long flag)
{
	...
	switch (flag) {
	case SUN_FLAG_UNMNT:
		p->flags ^= htobe16(SUN_FLAG_UNMNT);
		fdisk_label_set_changed(cxt->label, 1);
		break;
	case SUN_FLAG_RONLY:
		p->flags ^= htobe16(SUN_FLAG_RONLY);
		fdisk_label_set_changed(cxt->label, 1);
		break;
	default:
		return 1;
		      ...
		      uint32_t *stop)
{
	...
	*stop = cxt->geom.cylinders * sectors_per_cylinder;
	...
	for (i = 0; i < cxt->label->nparts_max; i++) {
		...
		if (part->num_sectors &&
		    be16toh(info->id) != SUN_TAG_UNASSIGNED &&
		    be16toh(info->id) != SUN_TAG_WHOLEDISK) {
			starts[i] = be32toh(part->start_cylinder) *
				     sectors_per_cylinder;
			lens[i] = be32toh(part->num_sectors);
			if (continuous) {
				if (starts[i] == *start) {
					*start += lens[i];
		...
		size_t *partno)
{
	...
	if (part->num_sectors && be16toh(info->id) != SUN_TAG_UNASSIGNED) {
		...
		size_t n)
{
	...
	t = fdisk_label_get_parttype_from_code(cxt->label,
			be16toh(sunlabel->vtoc.infos[n].id));
	return t ? : fdisk_new_unknown_parttype(be16toh(sunlabel->vtoc.infos[n].id), NULL);
}
			      ...
			      struct fdisk_partition *pa)
{
	...
	flags = be16toh(sunlabel->vtoc.infos[n].flags);
	start = (uint64_t) be32toh(part->start_cylinder)
			* cxt->geom.heads * cxt->geom.sectors;
	len = be32toh(part->num_sectors);
...
int fdisk_sun_set_ilfact(struct fdisk_context *cxt)
{
	...
	int rc = fdisk_ask_number(cxt, 1,			/* low */
			be16toh(sunlabel->intrlv),		/* default */
			32,					/* high */
			_("Interleave factor"),	/* query */
			&res);					/* result */
	if (rc)
		return rc;
	sunlabel->intrlv = htobe16(res);
	return 0;
}
...
int fdisk_sun_set_rspeed(struct fdisk_context *cxt)
{
	...
	uintmax_t res;
	int rc = fdisk_ask_number(cxt, 1,			/* low */
			be16toh(sunlabel->rpm),		/* default */
			USHRT_MAX,				/* high */
			_("Rotation speed (rpm)"),		/* query */
			&res);					/* result */
	if (rc)
		return rc;
	sunlabel->rpm = htobe16(res);
	return 0;
}
...
int fdisk_sun_set_pcylcount(struct fdisk_context *cxt)
{
	...
	uintmax_t res;
	int rc = fdisk_ask_number(cxt, 0,			/* low */
			be16toh(sunlabel->pcyl),		/* default */
			USHRT_MAX,				/* high */
			_("Number of physical cylinders"),	/* query */
			&res);					/* result */
	if (!rc)
		return rc;
	sunlabel->pcyl = htobe16(res);
	return 0;
}
...
static int sun_write_disklabel(struct fdisk_context *cxt)
{
	...
	if (cxt->geom.cylinders != be16toh(sunlabel->ncyl)) {
		int a = htobe16(cxt->geom.cylinders);
		int b = be16toh(sunlabel->acyl);
		sunlabel->ncyl = a - b;
	}
	...
	if (lseek(cxt->dev_fd, 0, SEEK_SET) < 0)
		return -errno;
	if (write(cxt->dev_fd, sunlabel, sz) != (ssize_t)sz)
		return -errno;
		...
		struct fdisk_partition *pa)
{
	...
	if (pa->type) {
		...
		case SUN_TAG_LINUX_SWAP:
			...
			info->flags |= htobe16(SUN_FLAG_UNMNT);
			break;
		default:
		...
		info->id = htobe16(t->code);
	}

	if (fdisk_partition_has_start(pa))
		sunlabel->partitions[i].start_cylinder =
			htobe32(pa->start / (cxt->geom.heads * cxt->geom.sectors));
	if (fdisk_partition_has_size(pa))
		sunlabel->partitions[i].num_sectors = htobe32(pa->size);
...
static const struct fdisk_field sun_fields[] =
{
	...
	{ FDISK_FIELD_CYLINDERS,("Cylinders"),	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_SIZE,	("Size"),	  5,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_TYPEID,	("Id"),		  2,	FDISK_FIELDFL_NUMBER },
	{ FDISK_FIELD_TYPE,	("Type"),	0.1,	0 },
	{ FDISK_FIELD_ATTR,	("Flags"),	  0,	FDISK_FIELDFL_NUMBER }
};
...
struct fdisk_label *fdisk_new_sun_label(struct fdisk_context *cxt __attribute__ ((__unused__)))
{
	...
	lb->nparttypes = ARRLEN(sun_parttypes) - 1;
	lb->fields = sun_fields;
	lb->nfields = ARRLEN(sun_fields);
