 ...
 */
static int userspace_event_verify(struct libmnt_monitor *mn __attribute__((__unused__)),
					struct monitor_entry *me)
{
...
int mnt_monitor_enable_userspace(struct libmnt_monitor *mn, int enable, const char *filename)
{
	...
	return monitor_modify_epoll(mn, me, 1);
err:
	rc = -errno;
	free_monitor_entry(me);
	return rc;
...
static int kernel_monitor_get_fd(struct libmnt_monitor *mn __attribute__((__unused__)),
				 struct monitor_entry *me)
{
	int rc;
...
int mnt_monitor_get_fd(struct libmnt_monitor *mn)
{
	...
	while (monitor_next_entry(mn, &itr, &me) == 0) {
		if (!me->enable)
			continue;
		rc = monitor_modify_epoll(mn, me, 1);
		if (rc)
			goto err;
	}
