...
static bool msg_click(uint16_t id,const char*text,void*user_data __attribute__((unused))){
	if(id==0){
		...
		EFI_STATUS st;
		EfiInitializeFwVolDevicepathNode(&fn,&gLinuxSimpleMassStorageGuid);
...
static int mass_draw(struct gui_activity*act){
	msgbox_create_yesno(msg_click,"Exit GUI Application and enter Linux Simple Mass Storage?");
	return -10;
}
